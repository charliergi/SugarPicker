...
in
   Map = map(ru: [translate(dx:10.0 dy:10.0 1:[scale(rx:10.0 ry:10.0 1:[primitive(kind:road) primitive(kind:road)])])] pu:[primitive(kind:pokemon)]) %% TODO change the map here
   ...
   fun{MyFunction Map}
	 ...
	 fun{Separate Map}
	    ...
	    [] map(ru:Ru pu:Pu) then
		  ...
		  {BuildFunc {Reorganise [I1 I2]} Time}
	       end
	    end
	 end
	 {Separate Map}
